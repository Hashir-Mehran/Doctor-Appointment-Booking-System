        ...
        {/* ✅ Fixed Profile Section */}
{
  token ? (
    <div
      className="relative flex items-center gap-2 cursor-pointer profile-menu select-none"
      onClick={() => setOpenDropdown(!openDropdown)} // works for mobile + click
      onMouseEnter={() => !isMobile && setOpenDropdown(true)} // hover for desktop
      onMouseLeave={() => !isMobile && setOpenDropdown(false)} // leave for desktop
    >
      ...
      {openDropdown && (
        <div
          className="absolute right-0 top-8 min-w-48 bg-stone-100 rounded-lg shadow-md flex flex-col gap-3 p-4 text-sm font-medium text-gray-700 z-50 border border-gray-200"
        >
          <p
            onClick={() => {
              navigate('my-profile');
      ...
      Create account
        ...
        <div className={`md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all ${showMenu ? 'fixed w-full' : 'h-0 w-0'}`}>
          <div className='flex items-center justify-between px-5 py-6'>
            <img className='w-36' onClick={() => { navigate('/'); setShowMenu(false) }} src={assets.logo} alt="" />
